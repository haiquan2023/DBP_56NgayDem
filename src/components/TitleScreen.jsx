...
import { hasSavedGame, clearSavedGame } from '../context/GameContext';
...
export default function TitleScreen({ onStart }) {
    ...
    const handleNewGame = () => {
        if (savedExists) {
            if (window.confirm('Bắt đầu lại từ đầu? Tiến trình hiện tại sẽ bị xóa.')) {
                clearSavedGame();
                onStart();
            }
        } else {
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center justify-start pt-14 md:pt-20">
                ...
                >
                    {/* Nút Tiếp tục - chỉ hiện khi có saved game */}
                    {savedExists && (
                        <motion.button
                    ...
                    {/* Nút Bắt đầu / Bắt đầu mới */}
                    <motion.button
                        whileHover={{ scale: 1.06 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={handleNewGame}
                        className={savedExists
                            ? "px-8 py-2.5 bg-white/5 border border-white/20 hover:border-brass-400/50 rounded-xl text-gray-400 hover:text-brass-300 font-serif-vn text-sm font-medium tracking-widest transition-all duration-400"
                            : "px-10 py-3.5 bg-gradient-to-r from-brass-500/30 via-brass-400/40 to-brass-500/30 border border-brass-300 rounded-xl text-brass-200 font-serif-vn text-lg font-bold tracking-widest transition-all duration-400 shadow-[0_0_18px_rgba(212,163,50,0.45)] hover:shadow-[0_0_26px_rgba(243,216,136,0.7)] hover:from-brass-500/50 hover:to-brass-500/50"
                        }
                        id="start-button"
                    >
                        {savedExists ? 'BẮT ĐẦU MỚI' : 'BẮT ĐẦU'}
                    </motion.button>
                </motion.div>
